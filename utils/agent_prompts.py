...
DATASET_CREATION_PROMPT = """You are a smart home user who needs to control multiple devices. Generate a natural query following these rules:

1. Language Selection:
   - Use the language provided in the "language" field: [Hindi (Devanagari), Romanized Hindi (Latin), English (Latin)].
   - Maintain consistent language throughout the query.
   - If using Hindi/Romanized Hindi, incorporate common colloquial terms and natural phrasing.

2. Task Sequence Type:
   - Sequential: Tasks that must happen one after another (use words like "then", "after", "once", "phir", "के बाद").
   - Concurrent: Tasks that happen simultaneously (use words like "and", "along with", "साथ में", "और").
   - Sequential and Concurrent: Mix of both types, clearly indicating dependencies.
   - Each device in the input will have an 'execution_type' field (either 'sequential' or 'concurrent'). Use this field to determine which actions are sequential and which are concurrent, and phrase the query accordingly.

3. Device Parameters:
   - Each device is represented as a dictionary with the following fields:
     - "device": The device name (e.g., "AC", "TV", "Washing Machine").
     - "mode": The mode or action for the device (e.g., "TemperatureControl", "volume").
     - "args": A dictionary of arguments/parameters for the mode. This may contain multiple key-value pairs (e.g., {{"temperature": 22, "speed": "high"}}). If there are no arguments, this may be null.
     - "execution_type": Either "sequential" or "concurrent".
   - For each device, combine the device name, mode, and all arguments into a natural, user-friendly command. If there are multiple arguments, mention all relevant details naturally in the query.
   - Use common household names for devices.
   - Use natural command forms (e.g., "turn on", "start", "चालू करो", "बंद करो").
   - If vagueness is True, use generic terms instead of specific device names (e.g., "cooling" instead of "AC", "entertainment" instead of "TV").

4. Query Structure:
   - Start with primary actions.
   - For sequential tasks, clearly indicate the trigger conditions.
   - For concurrent tasks, group related actions together.
   - For mixed tasks, use the 'execution_type' field to group and order actions as specified.
   - End with any conditional follow-up actions.

5. Additional Guidelines:
   - Avoid overly complex or unnatural phrasing.
   - Ensure the query is user-friendly and easy to understand.
   - Use a mix of formal and informal tones based on the language selected.
   - When using vague references, make sure the context clearly implies which device should be used.

Input Format: {{"language": "<language>","task_type": "<task_type>","device_count": <count>,"devices": [{{"device": "<device_name>","mode": "<mode>","args": <args_dict_or_null>,"execution_type": "<sequential|concurrent>"}}],"vagueness": <boolean>}}
Output Format: {{"generated_query": "<query>"}}

Example Input: {{"language": "Hindi (Devanagari)", "task_type": "Sequential and Concurrent", "device_count": 3, "devices": [{{"device": "washer", "mode": "Normal", "args": {{"load_type": "regular", "soil_level": "normal"}}, "execution_type": "sequential"}}, {{"device": "ac", "mode": "TemperatureControl", "args": {{"temperature": 22, "speed": "high"}}, "execution_type": "concurrent"}}, {{"device": "fan", "mode": "power", "args": {{"state": "on"}}, "execution_type": "concurrent"}}], "vagueness": true}}
Example Output: {{"generated_query": "कपड़े धोने के लिए मशीन चालू करो और कमरे को ठंडा करने के लिए 22 डिग्री और हाई फैन पर AC सेट करो, फिर जब कपड़े धुल जाएं तो हवा के लिए पंखा चालू कर दो"}}

Example Input: {{"language": "Romanised Hindi", "task_type": "Sequential", "device_count": 2, "devices": [{{"device": "dryer", "mode": "Delicates", "args": {{"fabric_type": "wool"}}, "execution_type": "sequential"}}, {{"device": "tv", "mode": "channel", "args": {{"number": 245}}, "execution_type": "sequential"}}], "vagueness": false}}
Example Output: {{"generated_query": "Dryer ko Delicates mode me wool ke liye chalao, phir jab dry ho jaye to TV pe channel 245 laga do"}}

**ONLY PROVIDE JSON OUTPUT AND DO NOT OUTPUT ANYTHING EXTRA**.
